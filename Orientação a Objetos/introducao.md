...
Orientação a objetos é uma paradigma aplicado na programação que consiste na interação entre diversas unidades de objetos.

Depois desta definição formal, você deve estar se perguntando onde e quando podemos utilizar a orientação a objetos? É normal este questionamento quando ficamos apenas na abstração.

Porém, se tem algo que é base de todo programador, é a orientação a objetos, um paradigma criado há anos atrás e usado até hoje, nas principais linguagens e tecnologias do mercado.

Usamos orientação a objetos para nos basear na vida real e resolver problemas de software, ou pelo menos tentamos. Ela acaba sendo uma base inclusive para outros paradigmas.

Orientação à objetos é algo atemporal e não está ligado a uma linguagem, o que significa que você pode investir seu tempo e aprender pois não vai mudar.
...
É comum ouvirmos o termo "encapsular" no dia-a-dia. "Encapsula isto em uma classe", mas o que de fato da significado a teste termo?
...
Não sabemos onde a variável é usada ou modificada, não sabemos sequer quais funções são pertinentes a idade ou a que se refere esta idade.
...
**As classes na orientação a objetos funcionam como um molde para os objetos. Os objetos são criados a partir de uma classe em muitos deles podem ser feitos da mesma classe.

### Herança

Assim como na vida real somos um acúmulo de características de nossos antepassados, na orientação a objetos isto também é possível. Na verdade isto é uma boa prática.
...
No exemplo anterior temos o cálculo do dígito verificador. Isto é exposo tanto pela propriedade **digitoVerificador** quanto pelo método **CalcularDigitoVerificador** nesta classe.
...
Vamos tomar como base o método **PodeSerPago** que nos informa se uma pagamento está vencido ou não, se pode ser pago ou não.
...
Porém, os pagamentos via boleto tem um formato diferente onde caso a data de vnecimento seja em um fim de semana ou feriado, o mesmo poderá ser pago no próximo dia útil.

Não queremos reescrever a mesma regra de pagamento duas vezes, queremos que ela seja padrão para todos os pagamentos, porém, queremos poder sobreescreve-la caso haja necessidade.
...
https://www.youtube.com/watch?v=o5g986Wpd18